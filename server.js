...
io.sockets.on('connection',function(socket){
	...
	/* payload:
		...
		join_room_response:
		{
			...
			'socket_id': the socket id of the persn that joined,
	...
	*/
	...
	socket.on('join_room',function(payload){
		...
		/*tell everyone that's in the room that someone just joined */
	...
	*/
	...
	socket.on('send_message',function(payload){
		log('A server received a command', 'send_message',payload);
		if(('undefined' === typeof payload) || !payload){
	...
	*/
	...
	socket.on('invite',function(payload){
		...
		/*check that a message can be traced to a username */
		var username = players[socket.id].username;
		if(('undefined' === typeof username) || !username){
...
/*check that a message can be traced to a username */
	...
	*/
	...
	socket.on('play_token',function(payload){
		...
		/*check that a player has previously registered */
		...
		var username = players[socket.id].username;
		if(('undefined' === typeof username) || !username){
			var error_message = 'play_token can\'t recognize who sent the message';
			log(error_message);
			socket.emit('play_token_response', {
												result: 'fail',
												message: error_message
